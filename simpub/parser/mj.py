...
class MjModelParser:
    def __init__(self, mj_model, visual_groups : set = None):
        self.parse_model(mj_model, visual_groups=visual_groups or { 1 })

    def parse(self):
        return self.sim_scene

    def parse_model(self, mj_model, visual_groups : set):
        sim_scene = SimScene()
        ...
        for geom_id in range(mj_model.ngeom):
            ...
            goem_group = mj_model.geom_group[geom_id].item()
            if goem_group not in visual_groups: continue
    ...
    def process_meshes(self, mj_model):
        ...
        for mesh_id in range(mj_model.nmesh):
            ...
            norms = mj_model.mesh_normal[start_vert:start_vert + num_verts]
